...
ui <- {
  fluidPage(
    titlePanel("ynabR - Monthly Spending"),
    sidebarLayout(
      position = "right",
      sidebarPanel(## category name input
        {
          selectInput(
...
server <- function(input, output) {
  ...
  df_of_interest <- reactive({
    ...
    if (input$ip_exclude == TRUE) {
      df_of_interest_stats <-
        df_of_interest[df_of_interest$activity != 0,]
    } else {
    ...
    activity_stats$mean <-
      mean(df_of_interest_stats$activity, na.rm = TRUE)
    activity_stats$median <-
      median(df_of_interest_stats$activity, na.rm = TRUE)
    activity_stats$min <-
      min(df_of_interest_stats$activity, na.rm = TRUE)
    activity_stats$max <-
      max(df_of_interest_stats$activity, na.rm = TRUE)
  ...
  output$plotly <- renderPlotly({
    ...
    p1 <-
      ...
      layout(annotations = c(annotations, list(
        text = paste('Median'),
        y = (activity_stats$median)
      )))
    ...
    p1 %>% layout(
      ...
      yaxis = list(
        ...
        tickprefix = "$",
        range = c(0, max(
          100,  ceiling(activity_stats$max / 50) * 50
        ))
      )
    )
  })
